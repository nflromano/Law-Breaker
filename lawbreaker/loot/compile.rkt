...
(require "ast.rkt" "types.rkt" "lambdas.rkt" "fv.rkt" "compile-ops.rkt" a86/ast "ccall.rkt")
...
(define (compile p)
  (match p
    [(Prog ds e)
     (prog (externs p) ;; externs modified to add ccall externs
           (Global 'entry)
           (Label 'entry)
           (Push rbx)    ; save callee-saved register	   
           (Mov rbx rdi) ; recv heap pointer
           (compile-defines-values ds) ;; allocates space for the defines in the heap
           (compile-e e (reverse (define-ids ds)) #f)
           (Add rsp (* 8 (length ds))) ;; pop function definitions
           (Pop rbx)     ; restore callee-save register
           (Ret)
           ;; compile all the code for lambda execution (heap is already allocated by the time these will be executed)
           (compile-defines ds)
      ...
      [(Lam f xs e)
       (let ((env  (append (reverse fvs) (reverse xs) (list #f))))
         (seq (Label (symbol->label f))              
              ...
              (Xor rax type-proc)
              (copy-env-to-stack fvs 8) ;; moves captured variables from heap to end of stack
              ;; (after this, the stack is in this order: return value, pointer to lambda in heap, non-free vars, captured env vars)
              ;; outside of the captured envirponment, everything is done in the original compile-e of the program, so those
              ;; first three are on the stack already when we enter this code
              (compile-e e env #t) ;; executes the lambda
...
(define (compile-app-nontail e es c)
  (let ((r (gensym 'ret))
    ...
    (seq (Lea rax r)
         ...
         (assert-proc rax)
         (Xor rax type-proc)
         (Mov rax (Offset rax 0)) ; fetch the code label FROM THE HEAP
         (Jmp rax)
         (Label r))))
...
(define (compile-defines-values ds)
  (seq (alloc-defines ds 0) ;; creates space in heap for code addr and each free variable, as well as pushing a pointer to each onto the stack
       (init-defines ds (reverse (define-ids ds)) 8) ;; assigns each free varialbe space in the heap with a pointer back to the top of that define entry on the heap
       (add-rbx-defines ds 0))) ;; adjusts rbx to be at the end of the data we just inserted
    ...
    [(cons (Defn f xs e) ds)
     (let ((fvs (fv (Lam f xs e))))
       (seq (Lea rax (symbol->label f))
            ...
            ;; after this, looks like (assume lam1 and lam2 have two free variables)
            ;; stack: | lam1 heap addr | lam2 heap addr | ... |
            ;; heap: | lam1 code addr | <empty> | <empty> | lam2 code addr | <empty> | <empty> | ...
...
(define (init-defines ds c off)
  (match ds
    ...
    ;; after this, looks like (assume lam1 and lam2 have two free variables)
            ;; stack: | lam1 heap addr | lam2 heap addr | ... |
            ;; heap: | lam1 code addr | lam1 heap addr | lam1 heap addr | lam2 code addr | lam2 heap addr | lam2 heap addr | ...
...
(define (add-rbx-defines ds n)
  (match ds
    ...
    [(cons (Defn f xs e) ds)
     ...
     ;; does affect the makeup of the heap or stack, just adjusts rbx to be at the end of the data rather than the beginning
...
(define (compile-lam f xs e c) 
  (let ((fvs (fv (Lam f xs e))))
    (seq (Lea rax (symbol->label f))
         (Mov (Offset rbx 0) rax)
         (free-vars-to-heap fvs c 8) ;; copy values from the stack from free variables into our lambda heap entry
         (Mov rax rbx) ; return value
         (Or rax type-proc)       ;; gets us a value is the stack that points to the lambda location in heap  
         (Add rbx (* 8 (add1 (length fvs))))))) ;; adjust heap pointer to be after captured env
...
(define (free-vars-to-heap fvs c off)
  (match fvs
    ['() (seq)]
    [(cons x fvs)
     (seq (Mov r8 (Offset rsp (lookup x c))) ;;r8 is the pointer to where lam is on the heap
          (Mov (Offset rbx off) r8) ;; put it where that free variables space is on the heap
          (free-vars-to-heap fvs c (+ off 8)))])) ;; continue to the next free variable until we run out of free variables
